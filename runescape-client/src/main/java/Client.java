		...
		if (hadNetworkError) { // L: 2916
			...
			if (gameState == 30) { // L: 2925
				...
				while (class153.method3318()) { // L: 2926
					var14 = class120.getPacketBufferNode(ClientPacket._REFLECTION_CHECK_REPLY, packetWriter.isaacCipher); // L: 2927
				...
				if (timer.field4265) { // L: 2934
					var14 = class120.getPacketBufferNode(ClientPacket._LOGIN_STATISTICS, packetWriter.isaacCipher); // L: 2936
						...
						for (var7 = 0; var7 < class268.mouseRecorder.index && (var15 == null || var15.packetBuffer.offset - var3 < 246); ++var7) { // L: 2954 2955
							...
							if (var9 != field505 || var8 != field506) { // L: 2963
								if (var15 == null) { // L: 2966
									var15 = class120.getPacketBufferNode(ClientPacket._EVENT_MOUSE_MOVE, packetWriter.isaacCipher); // L: 2967
				...
				if (MouseHandler.MouseHandler_lastButton == 1 || !Renderable.mouseCam && MouseHandler.MouseHandler_lastButton == 4 || MouseHandler.MouseHandler_lastButton == 2) { // L: 3034
					...
					var18 = class120.getPacketBufferNode(ClientPacket._EVENT_MOUSE_CLICK, packetWriter.isaacCipher); // L: 3045
				...
				if (KeyHandler.field141 > 0) { // L: 3051
					var14 = class120.getPacketBufferNode(ClientPacket._EVENT_KEYBOARD, packetWriter.isaacCipher); // L: 3052
				...
				if (field543 && field630 <= 0) { // L: 3068
					...
					var14 = class120.getPacketBufferNode(ClientPacket._EVENT_CAMERA_POSITION, packetWriter.isaacCipher); // L: 3072
				...
				if (class92.hasFocus && !hadFocus) { // L: 3077
					...
					var14 = class120.getPacketBufferNode(ClientPacket._EVENT_APPLET_FOCUS, packetWriter.isaacCipher); // L: 3080
				...
				if (!class92.hasFocus && hadFocus) { // L: 3084
					...
					var14 = class120.getPacketBufferNode(ClientPacket._EVENT_APPLET_FOCUS, packetWriter.isaacCipher); // L: 3087
					...
					if (packetWriter.field1349 > 750) { // L: 3128
						...
						if (Decimator.method1091() && KeyHandler.KeyHandler_pressedKeys[82] && KeyHandler.KeyHandler_pressedKeys[81] && mouseWheelRotation != 0) { // L: 3185
							...
							if (var3 != class67.localPlayer.plane) { // L: 3189
								...
								var18 = class120.getPacketBufferNode(ClientPacket._TELEPORT, packetWriter.isaacCipher); // L: 3194
													...
													do {
														...
														if (var36 == null) { // L: 3232
																...
																if (MouseHandler.MouseHandler_currentButton == 0) { // L: 3248
																	if (field763) { // L: 3249
																		if (User.dragInventoryWidget == SceneTilePaint.hoveredItemContainer && dragItemSlotSource != dragItemSlotDestination) { // L: 3250
																			...
																			PacketBufferNode var26 = class120.getPacketBufferNode(ClientPacket._OPHELD_D, packetWriter.isaacCipher); // L: 3286
															...
															if (Scene.shouldSendWalk()) { // L: 3314
																...
																PacketBufferNode var39 = class120.getPacketBufferNode(ClientPacket._MOVE_GAMECLICK, packetWriter.isaacCipher); // L: 3318
															...
															if (var3 > 15000 && var5 > 15000) { // L: 3376
																...
																var18 = class120.getPacketBufferNode(ClientPacket._IDLE_LOGOUT, packetWriter.isaacCipher); // L: 3380
															...
															if (packetWriter.pendingWrites > 50) { // L: 3385
																var18 = class120.getPacketBufferNode(ClientPacket._NO_TIMEOUT, packetWriter.isaacCipher); // L: 3387
	...
	final boolean method1447(PacketWriter var1) {
		...
		if (var2 == null) { // L: 5947
			...
			try {
				...
				int var10;
				int var20;
				if (ServerPacket._UPDATE_INV_FULL == var1.serverPacket) { // L: 5988
				...
				if (ServerPacket._OBJ_COUNT == var1.serverPacket) { // L: 6021
					SoundSystem.processZoneUpdate(ZoneUpdate.OBJ_COUNT); // L: 6022
					var1.serverPacket = null; // L: 6023
					return true; // L: 6024
				}

				if (ServerPacket._REBUILD_REGION_NORMAL == var1.serverPacket) { // L: 6026
					class114.loadRegions(false, var1.packetBuffer); // L: 6027
					var1.serverPacket = null; // L: 6028
					return true; // L: 6029
				}

				if (ServerPacket._UNKNOWN2 == var1.serverPacket) { // L: 6031
					return this.method1206(var1, 2); // L: 6032
				}

				if (ServerPacket._MAPPROJ_ANIM == var1.serverPacket) { // L: 6034
					SoundSystem.processZoneUpdate(ZoneUpdate.MAPPROJ_ANIM); // L: 6035
					var1.serverPacket = null; // L: 6036
					return true; // L: 6037
				}

				if (ServerPacket._NPC_INFO_SMALL == var1.serverPacket) { // L: 6039
				...
				if (ServerPacket._VARP_LARGE == var1.serverPacket) { // L: 6045
				...
				int var15;
				int var16;
				if (ServerPacket._PROJANIM_SPECIFIC == var1.serverPacket) { // L: 6057
				...
				String var50;
				if (ServerPacket._MESSAGE_PRIVATE_ECHO == var1.serverPacket) { // L: 6102
				...
				long var30;
				String var32;
				if (ServerPacket._MESSAGE_PRIVATE == var1.serverPacket) { // L: 6109
				...
				if (ServerPacket._UPDATE_FRIENDLIST == var1.serverPacket) { // L: 6136
				...
				Widget var33;
				if (ServerPacket._IF_SETANGLE == var1.serverPacket) { // L: 6142
				...
				if (ServerPacket._UPDATE_SITESETTINGS == var1.serverPacket) { // L: 6157
				...
				String var64;
				if (ServerPacket._OPEN_URL == var1.serverPacket) { // L: 6162
				...
				if (ServerPacket._CHAT_FILTER_SETTINGS == var1.serverPacket) { // L: 6171
				...
				if (ServerPacket._HINT_ARROW == var1.serverPacket) { // L: 6177
				...
				if (ServerPacket._PREFETCH_GAMEOBJECTS == var1.serverPacket) { // L: 6210
					SoundSystem.processZoneUpdate(ZoneUpdate.PREFETCH_GAMEOBJECTS); // L: 6211
					var1.serverPacket = null; // L: 6212
					return true; // L: 6213
				}

				if (ServerPacket._IF_OPENTOP == var1.serverPacket) { // L: 6215
				...
				if (ServerPacket._UPDATE_ZONE_PARTIAL_FOLLOWS == var1.serverPacket) { // L: 6225
					class392.Zone_Y = var3.readUnsignedByteNeg(); // L: 6226
					ItemComposition.ZONE_X = var3.readUnsignedByteAdd(); // L: 6227
					var1.serverPacket = null; // L: 6228
					return true; // L: 6229
				}

				if (ServerPacket._SYNC_CLIENT_VARCACHE == var1.serverPacket) { // L: 6231
				...
				GameObject var41;
				byte var79;
				if (ServerPacket._MESSAGE_CLANCHANNEL == var1.serverPacket) { // L: 6242
				...
				if (ServerPacket._UPDATE_ZONE_PARTIAL_ENCLOSED == var1.serverPacket) { // L: 6278
					ItemComposition.ZONE_X = var3.readUnsignedByteNeg(); // L: 6279
					class392.Zone_Y = var3.readUnsignedByte(); // L: 6280

					while (var3.offset < var1.serverPacketLength) { // L: 6281
						var20 = var3.readUnsignedByte(); // L: 6282
						ZoneUpdate var77 = ItemComposition.method3911()[var20]; // L: 6283
						SoundSystem.processZoneUpdate(var77); // L: 6284
					}
				...
				Widget var71;
				if (ServerPacket._UPDATE_INV_CLEAR == var1.serverPacket) { // L: 6289
				...
				boolean var52;
				if (ServerPacket._MESSAGE_GAME == var1.serverPacket) { // L: 6300
				...
				if (ServerPacket._IF_SETANIM == var1.serverPacket) { // L: 6314
				...
				if (ServerPacket._REBUILD_REGION == var1.serverPacket) { // L: 6327
					class114.loadRegions(true, var1.packetBuffer); // L: 6328
					var1.serverPacket = null; // L: 6329
					return true; // L: 6330
				}

				if (ServerPacket._MIDI_SONG == var1.serverPacket) { // L: 6332
				...
				if (ServerPacket._MIDI_JINGLE == var1.serverPacket) { // L: 6339
				...
				if (ServerPacket._CHAT_FILTER_SETTINGS_PRIVATECHAT == var1.serverPacket) { // L: 6347
					class142.privateChatMode = ChatChannel.method2146(var3.readUnsignedByte()); // L: 6348
					var1.serverPacket = null; // L: 6349
					return true; // L: 6350
				}

				if (ServerPacket._LEGACY_NPC_INFO_SMALL == var1.serverPacket) { // L: 6352
				...
				Widget var91;
				if (ServerPacket._IF_SETOBJECT == var1.serverPacket) { // L: 6357
				...
				if (ServerPacket._PLAYER_INFO == var1.serverPacket) { // L: 6396
				...
				if (ServerPacket._FRIENDLIST_LOADED == var1.serverPacket) { // L: 6402
				...
				if (ServerPacket._UPDATE_INV_PARTIAL == var1.serverPacket) { // L: 6408
				...
				if (ServerPacket._CAM_RESET == var1.serverPacket) { // L: 6437
				...
				if (ServerPacket._VARCLAN == var1.serverPacket) { // L: 6443
				...
				if (ServerPacket._CAM_SHAKE == var1.serverPacket) { // L: 6451
				...
				boolean var76;
				if (ServerPacket._TRADING_POST_RESULTS == var1.serverPacket) { // L: 6464
				...
				if (ServerPacket._CLANCHANNEL_DELTA == var1.serverPacket) { // L: 6475
				...
				if (ServerPacket._PLAYER_SPOTANIM_SPECIFIC == var1.serverPacket) { // L: 6486
				...
				if (ServerPacket._IF_SETEVENTS == var1.serverPacket) { // L: 6516
				...
				if (ServerPacket._CLANSETTINGS_FULL == var1.serverPacket) { // L: 6532
				...
				if (ServerPacket._AREA_SOUND == var1.serverPacket) { // L: 6550
					SoundSystem.processZoneUpdate(ZoneUpdate.AREA_SOUND); // L: 6551
					var1.serverPacket = null; // L: 6552
					return true; // L: 6553
				}

				if (ServerPacket._CAM_LOOKAT == var1.serverPacket) { // L: 6555
				...
				if (ServerPacket._MAP_ANIM == var1.serverPacket) { // L: 6579
					SoundSystem.processZoneUpdate(ZoneUpdate.MAP_ANIM); // L: 6580
					var1.serverPacket = null; // L: 6581
					return true; // L: 6582
				}

				if (ServerPacket._TOGGLE_OCULUS_ORB == var1.serverPacket) { // L: 6584
				...
				if (ServerPacket._NPC_INFO_LARGE == var1.serverPacket) { // L: 6593
				...
				if (ServerPacket._OBJ_ADD == var1.serverPacket) { // L: 6599
					SoundSystem.processZoneUpdate(ZoneUpdate.OBJ_ADD); // L: 6600
					var1.serverPacket = null; // L: 6601
					return true; // L: 6602
				}

				if (ServerPacket._TRADING_POST_UPDATE == var1.serverPacket) { // L: 6604
				...
				if (ServerPacket._LOGOUT_FULL == var1.serverPacket) { // L: 6618
					ApproximateRouteStrategy.logOut(); // L: 6619
					var1.serverPacket = null; // L: 6620
					return false; // L: 6621
				}

				if (ServerPacket._UPDATE_ZONE_FULL_FOLLOWS == var1.serverPacket) { // L: 6623
					ItemComposition.ZONE_X = var3.readUnsignedByte(); // L: 6624
					class392.Zone_Y = var3.readUnsignedByteAdd(); // L: 6625

					for (var20 = class392.Zone_Y; var20 < class392.Zone_Y + 8; ++var20) { // L: 6626
						for (var5 = ItemComposition.ZONE_X; var5 < ItemComposition.ZONE_X + 8; ++var5) { // L: 6627
					...
					for (PendingSpawn var53 = (PendingSpawn)pendingSpawns.last(); var53 != null; var53 = (PendingSpawn)pendingSpawns.previous()) { // L: 6634 6635 6637
						if (var53.x >= class392.Zone_Y && var53.x < class392.Zone_Y + 8 && var53.y >= ItemComposition.ZONE_X && var53.y < ItemComposition.ZONE_X + 8 && var53.plane == Tiles.Client_plane) { // L: 6636
							var53.hitpoints = 0;
						}
					}
				...
				if (ServerPacket._IF_SETCOLOUR == var1.serverPacket) { // L: 6642
				...
				if (ServerPacket._CLANCHANNEL_FULL == var1.serverPacket) { // L: 6657
				...
				if (ServerPacket._IF1_MODELROTATE == var1.serverPacket) { // L: 6675
				...
				if (ServerPacket._IF_SETPOSITION == var1.serverPacket) { // L: 6684
				...
				if (ServerPacket._SET_PLAYER_OP == var1.serverPacket) { // L: 6701
				...
				if (ServerPacket._UPDATE_UID192 == var1.serverPacket) { // L: 6713
				...
				if (ServerPacket._RESET_ANIMS == var1.serverPacket) { // L: 6719
				...
				if (ServerPacket._GAMEFRAME_FULL == var1.serverPacket) { // L: 6729
				...
				if (ServerPacket._ENTER_FREECAM == var1.serverPacket) { // L: 6772
				...
				if (ServerPacket._IF_SETNPCHEAD == var1.serverPacket) { // L: 6778
				...
				if (ServerPacket._TRIGGER_ONDIALOGABORT == var1.serverPacket) { // L: 6790
				...
				if (ServerPacket._RESET_CLIENT_VARCACHE == var1.serverPacket) { // L: 6795
				...
				if (ServerPacket._UNKNOWN3 == var1.serverPacket) { // L: 6808
					SoundSystem.processZoneUpdate(ZoneUpdate.field3050); // L: 6809
					var1.serverPacket = null; // L: 6810
					return true; // L: 6811
				}

				String var46;
				if (ServerPacket._MESSAGE_FRIENDCHANNEL == var1.serverPacket) { // L: 6813
				...
				if (ServerPacket._LOGOUT_TRANSFER == var1.serverPacket) { // L: 6840
				...
				if (ServerPacket._LOGOUT == var1.serverPacket) { // L: 6853
				...
				if (ServerPacket._LOC_DEL == var1.serverPacket) { // L: 6859
					SoundSystem.processZoneUpdate(ZoneUpdate.LOC_DEL); // L: 6860
					var1.serverPacket = null; // L: 6861
					return true; // L: 6862
				}

				NPC var90;
				if (ServerPacket._NPC_ANIM_SPECIFIC == var1.serverPacket) { // L: 6864
				...
				if (ServerPacket._CLANSETTINGS_DELTA == var1.serverPacket) { // L: 6901
				...
				if (ServerPacket._MESSAGE_CLANCHANNEL_SYSTEM == var1.serverPacket) { // L: 6912
				...
				if (ServerPacket._UPDATE_IGNORELIST == var1.serverPacket) { // L: 6938
				...
				if (ServerPacket._VARCLAN_DISABLE == var1.serverPacket) { // L: 6945
					class29.field182 = null; // L: 6946
					var1.serverPacket = null; // L: 6947
					return true; // L: 6948
				}

				if (ServerPacket._UNKNOWN4 == var1.serverPacket) { // L: 6950
					return this.method1206(var1, 1); // L: 6951
				}

				if (ServerPacket._SET_MAP_FLAG == var1.serverPacket) { // L: 6953
				...
				if (ServerPacket._RUNCLIENTSCRIPT == var1.serverPacket) { // L: 6961
				...
				if (ServerPacket._IF_OPENSUB == var1.serverPacket) { // L: 6975
				...
				if (ServerPacket._UPDATE_STAT == var1.serverPacket) { // L: 6985
				...
				if (ServerPacket._CAM_MOVETO == var1.serverPacket) { // L: 6998
				...
				if (ServerPacket._UNKNOWN1 == var1.serverPacket) { // L: 7014
					return this.method1207(var1); // L: 7015
				}

				if (ServerPacket._LEGACY_NPC_INFO_LARGE == var1.serverPacket) { // L: 7017
					class386.updateNpcs(true, var3); // L: 7018
					var1.serverPacket = null; // L: 7019
					return true; // L: 7020
				}

				if (ServerPacket._UPDATE_RUNENERGY == var1.serverPacket) { // L: 7022
				...
				if (ServerPacket._SYNTH_SOUND == var1.serverPacket) { // L: 7029
				...
				if (ServerPacket._IF_SETPLAYERHEAD == var1.serverPacket) { // L: 7037
				...
				if (ServerPacket._OBJ_DEL == var1.serverPacket) { // L: 7046
					SoundSystem.processZoneUpdate(ZoneUpdate.OBJ_DEL); // L: 7047
					var1.serverPacket = null; // L: 7048
					return true; // L: 7049
				}

				if (ServerPacket._CAM_SMOOTHRESET == var1.serverPacket && isCameraLocked) { // L: 7051 7052
				...
				if (ServerPacket._UPDATE_REBOOT_TIMER == var1.serverPacket) { // L: 7063
				...
				if (ServerPacket._MAP_SPOTANIM_SPECIFIC == var1.serverPacket) { // L: 7069
				...
				if (ServerPacket._UPDATE_INV_STOP_TRANSMIT == var1.serverPacket) { // L: 7094
				...
				if (ServerPacket._HEAT_MAP == var1.serverPacket) { // L: 7101
				...
				if (ServerPacket._SPOTANIM_SPECIFIC == var1.serverPacket) { // L: 7110
				...
				if (ServerPacket._IF_SETMODEL == var1.serverPacket) { // L: 7136
				...
				if (ServerPacket._LOC_ADD_CHANGE == var1.serverPacket) { // L: 7148
					SoundSystem.processZoneUpdate(ZoneUpdate.LOC_ADD_CHANGE); // L: 7149
					var1.serverPacket = null; // L: 7150
					return true; // L: 7151
				}

				if (ServerPacket._UPDATE_RUNWEIGHT == var1.serverPacket) { // L: 7153
				...
				if (ServerPacket._LOC_ANIM == var1.serverPacket) { // L: 7160
					SoundSystem.processZoneUpdate(ZoneUpdate.LOC_ANIM); // L: 7161
					var1.serverPacket = null; // L: 7162
					return true; // L: 7163
				}

				if (ServerPacket._MINIMAP_TOGGLE == var1.serverPacket) { // L: 7165
					minimapState = var3.readUnsignedByte(); // L: 7166
					var1.serverPacket = null; // L: 7167
					return true; // L: 7168
				}

				if (ServerPacket._REFLECTION_CHECK == var1.serverPacket) { // L: 7170
					ServerPacket.readReflectionCheck(var3, var1.serverPacketLength); // L: 7171
					var1.serverPacket = null; // L: 7172
					return true; // L: 7173
				}

				if (ServerPacket._LOC_ANIM_SPECIFIC == var1.serverPacket) { // L: 7175
				...
				if (ServerPacket._VARP_SMALL == var1.serverPacket) { // L: 7246
				...
				if (ServerPacket._IF_CLOSESUB == var1.serverPacket) { // L: 7258
				...
				if (ServerPacket._VARCLAN_ENABLE == var1.serverPacket) { // L: 7269
					class29.field182 = new class393(class150.HitSplatDefinition_cached); // L: 7270
					var1.serverPacket = null; // L: 7271
					return true; // L: 7272
				}

				if (ServerPacket._IF_MOVESUB == var1.serverPacket) { // L: 7274
				...
				if (ServerPacket._IF_SETSCROLLPOS == var1.serverPacket) { // L: 7295
				...
				if (ServerPacket._IF_SETHIDE == var1.serverPacket) { // L: 7310
				...
				if (ServerPacket._SEND_PING == var1.serverPacket) { // L: 7321
					var20 = var3.readInt(); // L: 7322
					var5 = var3.readInt(); // L: 7323
					var21 = DynamicObject.getGcDuration(); // L: 7324
					PacketBufferNode var42 = class120.getPacketBufferNode(ClientPacket._SEND_PING_REPLY, packetWriter.isaacCipher); // L: 7326
				...
				if (ServerPacket._IF_SETTEXT == var1.serverPacket) { // L: 7335
			...
			if (MouseHandler.MouseHandler_currentButton == 0) { // L: 11433
				if (isDraggingWidget) { // L: 11434
					...
					if (draggedOnWidget != null && HitSplatDefinition.method3782(clickedWidget) != null) { // L: 11444
						PacketBufferNode var9 = class120.getPacketBufferNode(ClientPacket._IF_BUTTOND, packetWriter.isaacCipher); // L: 11446
